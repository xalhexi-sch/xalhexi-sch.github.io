...
export default function FileCard({
  ...
  // Construct direct CDN URL from Supabase public storage
  const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL || 'https://hkcgqfuyxehypazfpjew.supabase.co'
  const imageUrl = isImage
    ? `${supabaseUrl}/storage/v1/object/public/hub-files/${file.storage_path}`
    : null
    ...
    return (
      ...
      >
        <div className="flex items-center gap-3.5 min-w-0 pr-4">
          {imageUrl ? (
            <div className="w-10 h-10 rounded-xl overflow-hidden bg-black/40 border border-zinc-700/60 shrink-0">
              <img
                src={imageUrl}
                alt={file.file_name}
  ...
  return (
    <div
      onClick={() => onPreview(file)}
      className="group relative flex flex-col justify-between p-3.5 rounded-2xl bg-zinc-900/50 hover:bg-zinc-800/70 border border-zinc-800/80 hover:border-zinc-700 transition-all duration-200 cursor-pointer shadow-sm hover:shadow-xl hover:-translate-y-0.5 select-none overflow-hidden"
    >
      {imageUrl ? (
        <div className="relative w-full h-36 mb-3 rounded-xl overflow-hidden bg-black/50 border border-zinc-800/80 flex items-center justify-center">
          <img
            src={imageUrl}
            alt={file.file_name}
            onError={() => setImageError(true)}
          ...
          {isOwner && (
            <button
              ...
              title="Delete File"
              className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 p-1.5 bg-black/70 backdrop-blur-md hover:bg-red-500/80 text-zinc-300 hover:text-white rounded-lg transition-all shadow-md"
            >
      ...
      <div>
        <h4 className="text-xs font-semibold text-zinc-200 group-hover:text-white line-clamp-1 mb-1 transition-colors">
          {file.file_name}
        </h4>
